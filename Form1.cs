...
namespace FIT_Automation
{
    ...
    public partial class MainForm : Form
    {
        ...
        private bool isCheckingBoxingEnabledForSelection;
        public MainForm()
        {
            InitializeComponent();
    ...
    {
        TC11CheckBox, TC12CheckBox, TC13CheckBox, TC14CheckBox, TC15CheckBox, TC16CheckBox, TC17CheckBox, TC18CheckBox,
        TC110CheckBox, TC111CheckBox, TC112CheckBox, TC113CheckBox, TC114CheckBox, TC115CheckBox, TC116CheckBox, TC117CheckBox,
        TC118CheckBox, TC119CheckBox, TC120CheckBox, TC121CheckBox, TC122CheckBox, TC123CheckBox, TC124CheckBox, TC125CheckBox,
        TC126CheckBox, TC127CheckBox, TC128CheckBox, TC129CheckBox, TC130CheckBox, TC131CheckBox, TC132CheckBox, TC133CheckBox,
        TC134CheckBox, TC135CheckBox, TC136CheckBox, TC137CheckBox, TC138CheckBox, TC139CheckBox, TC140CheckBox, TC141CheckBox,
        TC142CheckBox, TC143CheckBox, TC144CheckBox, TC145CheckBox, TC146CheckBox, TC147CheckBox, TC148CheckBox, TC149CheckBox,
        TC150CheckBox, TC151CheckBox, TC152CheckBox, TC153CheckBox, TC154CheckBox, TC155CheckBox, TC156CheckBox, TC157CheckBox,
        TC158CheckBox, TC159CheckBox, TC160CheckBox, TC161CheckBox, TC162CheckBox, TC163CheckBox, TC164CheckBox, TC165CheckBox,
        TC166CheckBox, TC167CheckBox, TC168CheckBox, TC169CheckBox, TC170CheckBox, TC171CheckBox, TC172CheckBox, TC173CheckBox,
        TC174CheckBox, TC175CheckBox, TC176CheckBox, TC177CheckBox, TC178CheckBox, TC179CheckBox, TC180CheckBox, TC181CheckBox,
        TC182CheckBox, TC183CheckBox, TC184CheckBox, TC185CheckBox, TC186CheckBox, TC187CheckBox, TC188CheckBox, TC189CheckBox,
        TC190CheckBox, TC191CheckBox, TC192CheckBox, TC193CheckBox, TC194CheckBox, TC195CheckBox, TC196CheckBox, TC197CheckBox,
        TC198CheckBox, TC199CheckBox, TC200CheckBox, TC201CheckBox, TC202CheckBox, TC203CheckBox, TC204CheckBox, TC205CheckBox,
        TC206CheckBox, TC207CheckBox, TC208CheckBox, TC209CheckBox, TC210CheckBox, TC211CheckBox, TC212CheckBox
    };
        }
        ...
        void addToolTips()
        {
            ...
            this.toolTip1.SetToolTip(this.TC195BTN, "1. Send MMS from a VoWiFi attached device camping on Cellular to a VoWiFi attached device on AP mode");
            this.toolTip1.SetToolTip(this.TC196BTN, "1. Send MMS from a VoWiFi attached device camping on Cellular to a VoWiFi attached device camping on cellular");
            this.toolTip1.SetToolTip(this.TC197BTN, "1. Send MMS from a VoWiFi attached device camping on Cellular to a VoLTE device");
            this.toolTip1.SetToolTip(this.TC198BTN, "1. Send MMS from a VoLTE device to a VoWiFi attached device camping on cellular");
            this.toolTip1.SetToolTip(this.TC199BTN, "1. Initiate a call between the devices.\r\n2.  Send MMS from a VoWiFi attached device camping on Cellular to a VoWiFi attached device on AP mode");
            this.toolTip1.SetToolTip(this.TC200BTN, "1. Initiate a call between the devices.\r\n2.  Send MMS from a VoWiFi attached device camping on Cellular to a VoWiFi attached device camping on cellular");
            this.toolTip1.SetToolTip(this.TC201BTN, "1. Initiate a call between the devices.\r\n2.  Send MMS from a VoWiFi attached device camping on Cellular to the VoLTE device");
            this.toolTip1.SetToolTip(this.TC202BTN, "1. Send SMS from a VoWiFi attached device camping on Cellular to a VoWiFi device.");
            this.toolTip1.SetToolTip(this.TC203BTN, "1. Send SMS from a VoWiFi attached device camping on Cellular to a VoLTE device.");
            this.toolTip1.SetToolTip(this.TC204BTN, "1. Send SMS from a VoLTE device to a VoWiFi attached device camping on cellular");
            this.toolTip1.SetToolTip(this.TC205BTN, "1. Initiate a call between the devices.\r\n2.  Send SMS from a VoWiFi attached device camping on Cellular to a VoWiFi attached device on AP mode");
            this.toolTip1.SetToolTip(this.TC206BTN, "1. Initiate a call between the devices.\r\n2.  Send SMS from a VoWiFi attached device camping on Cellular to a VoWiFi attached device camping on cellular");
            this.toolTip1.SetToolTip(this.TC207BTN, "1. Initiate a call between the devices.\r\n2.  Send SMS from a VoWiFi attached device camping on Cellular to the VoLTE device");
            this.toolTip1.SetToolTip(this.TC208BTN, "1. Initiate a call between the devices.\r\n2.  Receive SMS from a VoLTE device");
            this.toolTip1.SetToolTip(this.TC209BTN, "1. Intiate a call from VoWiFi device to any other party.\r\n2. Receive call from a VoWiFi device while being on active call.");
            this.toolTip1.SetToolTip(this.TC210BTN, "1. Intiate a call from VoWiFi device to any other party.\r\n2. Receive call from a VOLTE device while being on active call.");
            this.toolTip1.SetToolTip(this.TC211BTN, "1) Place a call from DUT 1 to DUT 2. \r\n2) While the call between DUT 1 and DUT 2 is in progress, place a call from DUT 3 to DUT 1 and validate that the call waiting notification is received on DUT 1\r\n3) Hold current call and answer incoming call from DUT 3\r\n4) DUT2 is on hold, keep it on hold for 1 minute\r\n4) Ensure audio is ok between DUT1 and DUT3\r\n5) Now swap calls so DUT 3 is kept on hold, keep DUT 3 on hold for 1 minute\r\n6) Ensure audio is ok between DUT1 and DUT2\r\n7) Again swap calls so DUT2 goes on hold, keep DUT 2 on hold for 1 minute\r\n8) Ensure audio is ok between DUT1 and DUT3");
            this.toolTip1.SetToolTip(this.TC212BTN, "1) Place a call from DUT 1 to DUT 2. \r\n2) While the call between DUT 1 and DUT 2 is in progress, place a call from DUT 3 to DUT 1 and validate that the call waiting notification is received on DUT 1\r\n3) Hold current call and answer incoming call from DUT 3\r\n4) DUT2 is on hold, keep it on hold for 1 minute\r\n4) Ensure audio is ok between DUT1 and DUT3\r\n5) Now swap calls so DUT 3 is kept on hold, keep DUT 3 on hold for 1 minute\r\n6) Ensure audio is ok between DUT1 and DUT2\r\n7) Again swap calls so DUT2 goes on hold, keep DUT 2 on hold for 1 minute\r\n8) Ensure audio is ok between DUT1 and DUT3");
            ...
            this.toolTip1.SetToolTip(this.TC195CheckBox, "1. Send MMS from a VoWiFi attached device camping on Cellular to a VoWiFi attached device on AP mode");
            this.toolTip1.SetToolTip(this.TC196CheckBox, "1. Send MMS from a VoWiFi attached device camping on Cellular to a VoWiFi attached device camping on cellular");
            this.toolTip1.SetToolTip(this.TC197CheckBox, "1. Send MMS from a VoWiFi attached device camping on Cellular to a VoLTE device");
            this.toolTip1.SetToolTip(this.TC198CheckBox, "1. Send MMS from a VoLTE device to a VoWiFi attached device camping on cellular");
            this.toolTip1.SetToolTip(this.TC199CheckBox, "1. Initiate a call between the devices.\r\n2.  Send MMS from a VoWiFi attached device camping on Cellular to a VoWiFi attached device on AP mode");
            this.toolTip1.SetToolTip(this.TC200CheckBox, "1. Initiate a call between the devices.\r\n2.  Send MMS from a VoWiFi attached device camping on Cellular to a VoWiFi attached device camping on cellular");
            this.toolTip1.SetToolTip(this.TC201CheckBox, "1. Initiate a call between the devices.\r\n2.  Send MMS from a VoWiFi attached device camping on Cellular to the VoLTE device");
            this.toolTip1.SetToolTip(this.TC202CheckBox, "1. Send SMS from a VoWiFi attached device camping on Cellular to a VoWiFi device.");
            this.toolTip1.SetToolTip(this.TC203CheckBox, "1. Send SMS from a VoWiFi attached device camping on Cellular to a VoLTE device.");
            this.toolTip1.SetToolTip(this.TC204CheckBox, "1. Send SMS from a VoLTE device to a VoWiFi attached device camping on cellular");
            this.toolTip1.SetToolTip(this.TC205CheckBox, "1. Initiate a call between the devices.\r\n2.  Send SMS from a VoWiFi attached device camping on Cellular to a VoWiFi attached device on AP mode");
            this.toolTip1.SetToolTip(this.TC206CheckBox, "1. Initiate a call between the devices.\r\n2.  Send SMS from a VoWiFi attached device camping on Cellular to a VoWiFi attached device camping on cellular");
            this.toolTip1.SetToolTip(this.TC207CheckBox, "1. Initiate a call between the devices.\r\n2.  Send SMS from a VoWiFi attached device camping on Cellular to the VoLTE device");
            this.toolTip1.SetToolTip(this.TC208CheckBox, "1. Initiate a call between the devices.\r\n2.  Receive SMS from a VoLTE device");
            this.toolTip1.SetToolTip(this.TC209CheckBox, "1. Intiate a call from VoWiFi device to any other party.\r\n2. Receive call from a VoWiFi device while being on active call.");
            this.toolTip1.SetToolTip(this.TC210CheckBox, "1. Intiate a call from VoWiFi device to any other party.\r\n2. Receive call from a VOLTE device while being on active call.");
            this.toolTip1.SetToolTip(this.TC211CheckBox, "1) Place a call from DUT 1 to DUT 2. \r\n2) While the call between DUT 1 and DUT 2 is in progress, place a call from DUT 3 to DUT 1 and validate that the call waiting notification is received on DUT 1\r\n3) Hold current call and answer incoming call from DUT 3\r\n4) DUT2 is on hold, keep it on hold for 1 minute\r\n4) Ensure audio is ok between DUT1 and DUT3\r\n5) Now swap calls so DUT 3 is kept on hold, keep DUT 3 on hold for 1 minute\r\n6) Ensure audio is ok between DUT1 and DUT2\r\n7) Again swap calls so DUT2 goes on hold, keep DUT 2 on hold for 1 minute\r\n8) Ensure audio is ok between DUT1 and DUT3");
            this.toolTip1.SetToolTip(this.TC212CheckBox, "1) Place a call from DUT 1 to DUT 2. \r\n2) While the call between DUT 1 and DUT 2 is in progress, place a call from DUT 3 to DUT 1 and validate that the call waiting notification is received on DUT 1\r\n3) Hold current call and answer incoming call from DUT 3\r\n4) DUT2 is on hold, keep it on hold for 1 minute\r\n4) Ensure audio is ok between DUT1 and DUT3\r\n5) Now swap calls so DUT 3 is kept on hold, keep DUT 3 on hold for 1 minute\r\n6) Ensure audio is ok between DUT1 and DUT2\r\n7) Again swap calls so DUT2 goes on hold, keep DUT 2 on hold for 1 minute\r\n8) Ensure audio is ok between DUT1 and DUT3");
        ...
        private void TC195BTN_Click(object sender, EventArgs e)
        {
            if (DUTchkbx.CheckedItems.Count == 0)
            {
                MessageBox.Show("Please select a device to run TC 1.95.");
                return;
            }
            //string deviceId = devicechkbxlst.CheckedItems[0].ToString();
            string deviceId = DUTchkbx.CheckedItems.Count > 0 ? DUTchkbx.CheckedItems[0].ToString() : null;
            string refDeviceId = REFchekbx.CheckedItems.Count > 0 ? REFchekbx.CheckedItems[0].ToString() : null;
            string moCallerId = devicechkbxlst.CheckedItems.Count > 0 ? devicechkbxlst.CheckedItems[0].ToString() : null;
            TC_1_95 test = new TC_1_95(deviceId, outputRTB, TC195BTN, refDeviceId);
            test.RunTest();
        }

        private void TC196BTN_Click(object sender, EventArgs e)
        {
            if (DUTchkbx.CheckedItems.Count == 0)
            {
                MessageBox.Show("Please select a device to run TC 1.96.");
                return;
            }
            string deviceId = DUTchkbx.CheckedItems.Count > 0 ? DUTchkbx.CheckedItems[0].ToString() : null;
            string refDeviceId = REFchekbx.CheckedItems.Count > 0 ? REFchekbx.CheckedItems[0].ToString() : null;
            TC_1_96 test = new TC_1_96(deviceId, outputRTB, TC196BTN, refDeviceId);
            test.RunTest();
        }

        private void TC197BTN_Click(object sender, EventArgs e)
        {
            if (DUTchkbx.CheckedItems.Count == 0)
            {
                MessageBox.Show("Please select a device to run TC 1.97.");
                return;
            }
            string deviceId = DUTchkbx.CheckedItems.Count > 0 ? DUTchkbx.CheckedItems[0].ToString() : null;
            string refDeviceId = REFchekbx.CheckedItems.Count > 0 ? REFchekbx.CheckedItems[0].ToString() : null;
            TC_1_97 test = new TC_1_97(deviceId, outputRTB, TC197BTN, refDeviceId);
            test.RunTest();
        }

        private void TC198BTN_Click(object sender, EventArgs e)
        {
            if (DUTchkbx.CheckedItems.Count == 0)
            {
                MessageBox.Show("Please select a device to run TC 1.98.");
                return;
            }
            string deviceId = DUTchkbx.CheckedItems.Count > 0 ? DUTchkbx.CheckedItems[0].ToString() : null;
            string refDeviceId = REFchekbx.CheckedItems.Count > 0 ? REFchekbx.CheckedItems[0].ToString() : null;
            TC_1_98 test = new TC_1_98(deviceId, outputRTB, TC198BTN, refDeviceId);
            test.RunTest();
        }

        private void TC199BTN_Click(object sender, EventArgs e)
        {
            if (DUTchkbx.CheckedItems.Count == 0)
            {
                MessageBox.Show("Please select a device to run TC 1.99.");
                return;
            }
            string deviceId = DUTchkbx.CheckedItems.Count > 0 ? DUTchkbx.CheckedItems[0].ToString() : null;
            string refDeviceId = REFchekbx.CheckedItems.Count > 0 ? REFchekbx.CheckedItems[0].ToString() : null;
            TC_1_99 test = new TC_1_99(deviceId, outputRTB, TC199BTN, refDeviceId);
            test.RunTest();
        }

        private void TC200BTN_Click(object sender, EventArgs e)
        {
            if (DUTchkbx.CheckedItems.Count == 0)
            {
                MessageBox.Show("Please select a device to run TC 2.00.");
                return;
            }
            string deviceId = DUTchkbx.CheckedItems.Count > 0 ? DUTchkbx.CheckedItems[0].ToString() : null;
            string refDeviceId = REFchekbx.CheckedItems.Count > 0 ? REFchekbx.CheckedItems[0].ToString() : null;
            TC_2_00 test = new TC_2_00(deviceId, outputRTB, TC200BTN, refDeviceId);
            test.RunTest();
        }

        private void TC201BTN_Click(object sender, EventArgs e)
        {
            if (DUTchkbx.CheckedItems.Count == 0)
            {
                MessageBox.Show("Please select a device to run TC 2.01.");
                return;
            }
            string deviceId = DUTchkbx.CheckedItems.Count > 0 ? DUTchkbx.CheckedItems[0].ToString() : null;
            string refDeviceId = REFchekbx.CheckedItems.Count > 0 ? REFchekbx.CheckedItems[0].ToString() : null;
            TC_2_01 test = new TC_2_01(deviceId, outputRTB, TC201BTN, refDeviceId);
            test.RunTest();
        }

        private void TC202BTN_Click(object sender, EventArgs e)
        {
            if (DUTchkbx.CheckedItems.Count == 0)
            {
                MessageBox.Show("Please select a device to run TC 2.02.");
                return;
            }
            string deviceId = DUTchkbx.CheckedItems.Count > 0 ? DUTchkbx.CheckedItems[0].ToString() : null;
            string refDeviceId = REFchekbx.CheckedItems.Count > 0 ? REFchekbx.CheckedItems[0].ToString() : null;
            TC_2_02 test = new TC_2_02(deviceId, outputRTB, TC202BTN, refDeviceId);
            test.RunTest();
        }

        private void TC203BTN_Click(object sender, EventArgs e)
        {
            if (DUTchkbx.CheckedItems.Count == 0)
            {
                MessageBox.Show("Please select a device to run TC 2.03.");
                return;
            }
            string deviceId = DUTchkbx.CheckedItems.Count > 0 ? DUTchkbx.CheckedItems[0].ToString() : null;
            string refDeviceId = REFchekbx.CheckedItems.Count > 0 ? REFchekbx.CheckedItems[0].ToString() : null;
            TC_2_03 test = new TC_2_03(deviceId, outputRTB, TC203BTN, refDeviceId);
            test.RunTest();
        }

        private void TC204BTN_Click(object sender, EventArgs e)
        {
            if (DUTchkbx.CheckedItems.Count == 0)
            {
                MessageBox.Show("Please select a device to run TC 2.04.");
                return;
            }
            string deviceId = DUTchkbx.CheckedItems.Count > 0 ? DUTchkbx.CheckedItems[0].ToString() : null;
            string refDeviceId = REFchekbx.CheckedItems.Count > 0 ? REFchekbx.CheckedItems[0].ToString() : null;
            TC_2_04 test = new TC_2_04(deviceId, outputRTB, TC204BTN, refDeviceId);
            test.RunTest();
        }

        private void TC205BTN_Click(object sender, EventArgs e)
        {
            if (DUTchkbx.CheckedItems.Count == 0)
            {
                MessageBox.Show("Please select a device to run TC 2.05.");
                return;
            }
            string deviceId = DUTchkbx.CheckedItems.Count > 0 ? DUTchkbx.CheckedItems[0].ToString() : null;
            string refDeviceId = REFchekbx.CheckedItems.Count > 0 ? REFchekbx.CheckedItems[0].ToString() : null;
            TC_2_05 test = new TC_2_05(deviceId, outputRTB, TC205BTN, refDeviceId);
            test.RunTest();
        }

        private void TC206BTN_Click(object sender, EventArgs e)
        {
            if (DUTchkbx.CheckedItems.Count == 0)
            {
                MessageBox.Show("Please select a device to run TC 2.06.");
                return;
            }
            string deviceId = DUTchkbx.CheckedItems.Count > 0 ? DUTchkbx.CheckedItems[0].ToString() : null;
            string refDeviceId = REFchekbx.CheckedItems.Count > 0 ? REFchekbx.CheckedItems[0].ToString() : null;
            TC_2_06 test = new TC_2_06(deviceId, outputRTB, TC206BTN, refDeviceId);
            test.RunTest();
        }

        private void TC207BTN_Click(object sender, EventArgs e)
        {
            if (DUTchkbx.CheckedItems.Count == 0)
            {
                MessageBox.Show("Please select a device to run TC 2.07.");
                return;
            }
            string deviceId = DUTchkbx.CheckedItems.Count > 0 ? DUTchkbx.CheckedItems[0].ToString() : null;
            string refDeviceId = REFchekbx.CheckedItems.Count > 0 ? REFchekbx.CheckedItems[0].ToString() : null;
            TC_2_07 test = new TC_2_07(deviceId, outputRTB, TC207BTN, refDeviceId);
            test.RunTest();
        }

        private void TC208BTN_Click(object sender, EventArgs e)
        {
            if (DUTchkbx.CheckedItems.Count == 0)
            {
                MessageBox.Show("Please select a device to run TC 2.08.");
                return;
            }
            string deviceId = DUTchkbx.CheckedItems.Count > 0 ? DUTchkbx.CheckedItems[0].ToString() : null;
            string refDeviceId = REFchekbx.CheckedItems.Count > 0 ? REFchekbx.CheckedItems[0].ToString() : null;
            TC_2_08 test = new TC_2_08(deviceId, outputRTB, TC208BTN, refDeviceId);
            test.RunTest();
        }

        private void TC209BTN_Click(object sender, EventArgs e)
        {
            if (DUTchkbx.CheckedItems.Count == 0)
            {
                MessageBox.Show("Please select a device to run TC 2.09.");
                return;
            }
            //string deviceId = devicechkbxlst.CheckedItems[0].ToString();
            string deviceId = DUTchkbx.CheckedItems.Count > 0 ? DUTchkbx.CheckedItems[0].ToString() : null;
            string refDeviceId = REFchekbx.CheckedItems.Count > 0 ? REFchekbx.CheckedItems[0].ToString() : null;
            string moCallerId = devicechkbxlst.CheckedItems.Count > 0 ? devicechkbxlst.CheckedItems[0].ToString() : null;
            TC_2_09 test = new TC_2_09(deviceId, refDeviceId, moCallerId, outputRTB, TC209BTN);
            test.RunTest();
        }

        private void TC210BTN_Click(object sender, EventArgs e)
        {
            if (DUTchkbx.CheckedItems.Count == 0)
            {
                MessageBox.Show("Please select a device to run TC 2.10.");
                return;
            }
            //string deviceId = devicechkbxlst.CheckedItems[0].ToString();
            string deviceId = DUTchkbx.CheckedItems.Count > 0 ? DUTchkbx.CheckedItems[0].ToString() : null;
            string refDeviceId = REFchekbx.CheckedItems.Count > 0 ? REFchekbx.CheckedItems[0].ToString() : null;
            string moCallerId = devicechkbxlst.CheckedItems.Count > 0 ? devicechkbxlst.CheckedItems[0].ToString() : null;
            TC_2_10 test = new TC_2_10(deviceId, refDeviceId, moCallerId, outputRTB, TC210BTN);
            test.RunTest();
        }

        private void TC211BTN_Click(object sender, EventArgs e)
        {
            if (DUTchkbx.CheckedItems.Count == 0)
            {
                MessageBox.Show("Please select a device to run TC 2.11.");
                return;
            }
            //string deviceId = devicechkbxlst.CheckedItems[0].ToString();
            string deviceId = DUTchkbx.CheckedItems.Count > 0 ? DUTchkbx.CheckedItems[0].ToString() : null;
            string refDeviceId = REFchekbx.CheckedItems.Count > 0 ? REFchekbx.CheckedItems[0].ToString() : null;
            string moCallerId = devicechkbxlst.CheckedItems.Count > 0 ? devicechkbxlst.CheckedItems[0].ToString() : null;
            TC_2_11 test = new TC_2_11(deviceId, refDeviceId, moCallerId, outputRTB, TC211BTN);
            test.RunTest();
        }

        private void TC212BTN_Click(object sender, EventArgs e)
        {
            if (DUTchkbx.CheckedItems.Count == 0)
            {
                MessageBox.Show("Please select a device to run TC 2.12.");
                return;
            }
            //string deviceId = devicechkbxlst.CheckedItems[0].ToString();
            string deviceId = DUTchkbx.CheckedItems.Count > 0 ? DUTchkbx.CheckedItems[0].ToString() : null;
            string refDeviceId = REFchekbx.CheckedItems.Count > 0 ? REFchekbx.CheckedItems[0].ToString() : null;
            string moCallerId = devicechkbxlst.CheckedItems.Count > 0 ? devicechkbxlst.CheckedItems[0].ToString() : null;
            TC_2_12 test = new TC_2_12(deviceId, refDeviceId, moCallerId, outputRTB, TC212BTN);
            test.RunTest();
        }
        ...
        private async void ProcessTCBatchButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (TC205CheckBox.Checked) testCases.Add("TC 2.05");
                if (TC206CheckBox.Checked) testCases.Add("TC 2.06");
                if (TC207CheckBox.Checked) testCases.Add("TC 2.07");
                if (TC208CheckBox.Checked) testCases.Add("TC 2.08");
                if (TC209CheckBox.Checked) testCases.Add("TC 2.09");
                if (TC210CheckBox.Checked) testCases.Add("TC 2.10");
                if (TC211CheckBox.Checked) testCases.Add("TC 2.11");
                if (TC212CheckBox.Checked) testCases.Add("TC 2.12");
                ...
                "TC 1.16","TC 1.18","TC 1.19","TC 1.20","TC 1.21", "TC 1.25", "TC 1.26", "TC 1.27", "TC 1.30", "TC 1.31",
                        ...
                        "TC 1.55", "TC 1.56", "TC 1.57", "TC 1.58", "TC 1.69", "TC 1.70", "TC 1.71", "TC 1.91", "TC 1.95", "TC 1.96", "TC 1.97", "TC 1.98", "TC 1.99", "TC 2.00",
                        "TC 2.01", "TC 2.02", "TC 2.03", "TC 2.04", "TC 2.05", "TC 2.06", "TC 2.07", "TC 2.08"
            }.Contains(testCase))
                    {
                            ...
                            tasks.Add(Task.Run(async () =>
                            {
                                switch (testCase)
                                {
                                    ...
                                    case "TC 1.91":
                                        ...
                                        break;
                                    case "TC 1.95":
                                        new TC_1_95(dut, outputRTB, TC195BTN, refDev).RunTest();
                                        UpdateCheckBoxColor(TC195CheckBox, TC195BTN);
                                        break;
                                    case "TC 1.96":
                                        new TC_1_96(dut, outputRTB, TC196BTN, refDev).RunTest();
                                        UpdateCheckBoxColor(TC196CheckBox, TC196BTN);
                                        break;
                                    case "TC 1.97":
                                        new TC_1_97(dut, outputRTB, TC197BTN, refDev).RunTest();
                                        UpdateCheckBoxColor(TC197CheckBox, TC197BTN);
                                        break;
                                    case "TC 1.98":
                                        new TC_1_98(dut, outputRTB, TC198BTN, refDev).RunTest();
                                        UpdateCheckBoxColor(TC198CheckBox, TC198BTN);
                                        break;
                                    case "TC 1.99":
                                        new TC_1_99(dut, outputRTB, TC199BTN, refDev).RunTest();
                                        UpdateCheckBoxColor(TC199CheckBox, TC199BTN);
                                        break;
                                    case "TC 2.00":
                                        new TC_2_00(dut, outputRTB, TC200BTN, refDev).RunTest();
                                        UpdateCheckBoxColor(TC200CheckBox, TC200BTN);
                                        break;
                                    case "TC 2.01":
                                        new TC_2_01(dut, outputRTB, TC201BTN, refDev).RunTest();
                                        UpdateCheckBoxColor(TC201CheckBox, TC201BTN);
                                        break;
                                    case "TC 2.02":
                                        new TC_2_02(dut, outputRTB, TC202BTN, refDev).RunTest();
                                        UpdateCheckBoxColor(TC202CheckBox, TC202BTN);
                                        break;
                                    case "TC 2.03":
                                        new TC_2_03(dut, outputRTB, TC203BTN, refDev).RunTest();
                                        UpdateCheckBoxColor(TC203CheckBox, TC203BTN);
                                        break;
                                    case "TC 2.04":
                                        new TC_2_04(dut, outputRTB, TC204BTN, refDev).RunTest();
                                        UpdateCheckBoxColor(TC204CheckBox, TC204BTN);
                                        break;
                                    case "TC 2.05":
                                        new TC_2_05(dut, outputRTB, TC205BTN, refDev).RunTest();
                                        UpdateCheckBoxColor(TC205CheckBox, TC205BTN);
                                        break;
                                    case "TC 2.06":
                                        new TC_2_06(dut, outputRTB, TC206BTN, refDev).RunTest();
                                        UpdateCheckBoxColor(TC206CheckBox, TC206BTN);
                                        break;
                                    case "TC 2.07":
                                        new TC_2_07(dut, outputRTB, TC207BTN, refDev).RunTest();
                                        UpdateCheckBoxColor(TC207CheckBox, TC207BTN);
                                        break;
                                    case "TC 2.08":
                                        new TC_2_08(dut, outputRTB, TC208BTN, refDev).RunTest();
                                        UpdateCheckBoxColor(TC208CheckBox, TC208BTN);
                                        break;
                    ...
                    "TC 1.65", "TC 1.66", "TC 1.67", "TC 1.68", "TC 1.90", "TC 1.92", "TC 1.93", "TC 1.94", "TC 2.09", "TC 2.10", "TC 2.11", "TC 2.12"}.Contains(testCase))
                    {
                            ...
                            tasks.Add(Task.Run(() =>
                            {
                                switch (testCase)
                                {
                                    ...
                                    case "TC 1.93":
                                        ...
                                        break;
                                    case "TC 1.94":
                                        new TC_1_94(dut, refDev, moDev, outputRTB, TC194BTN).RunTest();
                                        UpdateCheckBoxColor(TC194CheckBox, TC194BTN);
                                        break;
                                    case "TC 2.09":
                                        new TC_2_09(dut, refDev, moDev, outputRTB, TC209BTN).RunTest();
                                        UpdateCheckBoxColor(TC209CheckBox, TC209BTN);
                                        break;
                                    case "TC 2.10":
                                        new TC_2_10(dut, refDev, moDev, outputRTB, TC210BTN).RunTest();
                                        UpdateCheckBoxColor(TC210CheckBox, TC210BTN);
                                        break;
                                    case "TC 2.11":
                                        new TC_2_11(dut, refDev, moDev, outputRTB, TC211BTN).RunTest();
                                        UpdateCheckBoxColor(TC211CheckBox, TC211BTN);
                                        break;
                                    case "TC 2.12":
                                        new TC_2_12(dut, refDev, moDev, outputRTB, TC212BTN).RunTest();
                                        UpdateCheckBoxColor(TC212CheckBox, TC212BTN);
                                        break;
                                }
                            }, _runCts.Token));
                        }
        ...
        */
                    ...
                    await Task.Run(async () =>
                        {
                            switch (id.ToUpperInvariant())
                            {
                                ...
                                case "1.95":
                                    {
                                        var t = new TC_1_95(dutId, outputRTB, TC195BTN, refId);
                                        t.RunTest();
                                        break;
                                    }
                                case "1.96":
                                    {
                                        var t = new TC_1_96(dutId, outputRTB, TC196BTN, refId);
                                        t.RunTest();
                                        break;
                                    }
                                case "1.97":
                                    {
                                        var t = new TC_1_97(dutId, outputRTB, TC197BTN, refId);
                                        t.RunTest();
                                        break;
                                    }
                                case "1.98":
                                    {
                                        var t = new TC_1_98(dutId, outputRTB, TC198BTN, refId);
                                        t.RunTest();
                                        break;
                                    }
                                case "1.99":
                                    {
                                        var t = new TC_1_99(dutId, outputRTB, TC199BTN, refId);
                                        t.RunTest();
                                        break;
                                    }
                                case "2.00":
                                    {
                                        var t = new TC_2_00(dutId, outputRTB, TC200BTN, refId);
                                        t.RunTest();
                                        break;
                                    }
                                case "2.01":
                                    {
                                        var t = new TC_2_01(dutId, outputRTB, TC201BTN, refId);
                                        t.RunTest();
                                        break;
                                    }
                                case "2.02":
                                    {
                                        var t = new TC_2_02(dutId, outputRTB, TC202BTN, refId);
                                        t.RunTest();
                                        break;
                                    }
                                case "2.03":
                                    {
                                        var t = new TC_2_03(dutId, outputRTB, TC203BTN, refId);
                                        t.RunTest();
                                        break;
                                    }
                                case "2.04":
                                    {
                                        var t = new TC_2_04(dutId, outputRTB, TC204BTN, refId);
                                        t.RunTest();
                                        break;
                                    }
                                case "2.05":
                                    {
                                        var t = new TC_2_05(dutId, outputRTB, TC205BTN, refId);
                                        t.RunTest();
                                        break;
                                    }
                                case "2.06":
                                    {
                                        var t = new TC_2_06(dutId, outputRTB, TC206BTN, refId);
                                        t.RunTest();
                                        break;
                                    }
                                case "2.07":
                                    {
                                        var t = new TC_2_07(dutId, outputRTB, TC207BTN, refId);
                                        t.RunTest();
                                        break;
                                    }
                                case "2.08":
                                    {
                                        var t = new TC_2_08(dutId, outputRTB, TC208BTN, refId);
                                        t.RunTest();
                                        break;
                                    }
                                case "2.09":
                                    {
                                        var t = new TC_2_09(dutId, refId, moCallerId, outputRTB, TC209BTN);
                                        t.RunTest();
                                        break;
                                    }
                                case "2.10":
                                    {
                                        var t = new TC_2_10(dutId, refId, moCallerId, outputRTB, TC210BTN);
                                        t.RunTest();
                                        break;
                                    }
                                case "2.11":
                                    {
                                        var t = new TC_2_11(dutId, refId, moCallerId, outputRTB, TC211BTN);
                                        t.RunTest();
                                        break;
                                    }
                                case "2.12":
                                    {
                                        var t = new TC_2_12(dutId, refId, moCallerId, outputRTB, TC212BTN);
                                        t.RunTest();
                                        break;
                                    }
        ...
        private void SelectAllTCsBTN_Click(object sender, EventArgs e)
        {
           ...
           TC11CheckBox.Checked = true;
            ...
            TC195CheckBox.Checked = true;
            TC196CheckBox.Checked = true;
            TC197CheckBox.Checked = true;
            TC198CheckBox.Checked = true;
            TC199CheckBox.Checked = true;
            TC200CheckBox.Checked = true;
            TC201CheckBox.Checked = true;
            TC202CheckBox.Checked = true;
            TC203CheckBox.Checked = true;
            TC204CheckBox.Checked = true;
            TC205CheckBox.Checked = true;
            TC206CheckBox.Checked = true;
            TC207CheckBox.Checked = true;
            TC208CheckBox.Checked = true;
            */
        }
        ...
        private void clearAllTCCheckBoxes()
        {
            ...
            TC195CheckBox.Checked = false;
            TC196CheckBox.Checked = false;
            TC197CheckBox.Checked = false;
            TC198CheckBox.Checked = false;
            TC199CheckBox.Checked = false;
            TC200CheckBox.Checked = false;
            TC201CheckBox.Checked = false;
            TC202CheckBox.Checked = false;
            TC203CheckBox.Checked = false;
            TC204CheckBox.Checked = false;
            TC205CheckBox.Checked = false;
            TC206CheckBox.Checked = false;
            TC207CheckBox.Checked = false;
            TC208CheckBox.Checked = false;
            */
        }
        ...
        private void CheckAllDUTREFAndMOOnlyBoxes_Click(object sender, EventArgs e)
        {
            ...
            TC193CheckBox.Checked = true;
            TC194CheckBox.Checked = true;
            TC209CheckBox.Checked = true;
            TC210CheckBox.Checked = true;
            TC211CheckBox.Checked = true;
            TC212CheckBox.Checked = true;
        }
        ...
        */
        ...
        // Check next 5 boxes in ascending order
        private void CheckNextFiveBoxesInAscOrderButton_Click(object sender, EventArgs e)
        {
            Upload.SelectTab(1);

            if (isCheckingBoxingEnabledForSelection)
            {
                // 2. Find the first unchecked checkbox
                int startIdx = allCheckBoxes.FindIndex(cb => !cb.Checked);
                if (startIdx == -1)
                    return; // All are checked

                // 3. Check up to 5 unchecked checkboxes from that position
                int checkedCount = 0;
                for (int i = startIdx; i < allCheckBoxes.Count && checkedCount < int.Parse(SelectTextBoxSelectionAmount.Text); i++)
                {
            ...
            {
                // instead of checking boxes you uncheck them in ascending order for the specified amount in the text box starting from the first checked box
                int startIdx = allCheckBoxes.FindIndex(cb => cb.Checked);
                if (startIdx == -1)
                    return; // All are checked

                // 3. Check up to 5 checked checkboxes from that position
                int unCheckedCount = 0;
                for (int i = startIdx; i < allCheckBoxes.Count && unCheckedCount < int.Parse(SelectTextBoxSelectionAmount.Text); i++)
                {
                    if (allCheckBoxes[i].Checked)
                    {
                        allCheckBoxes[i].Checked = false;
        ...
        private void CheckNextFiveBoxesInDescOrderButton_Click(object sender, EventArgs e)
        {
            Upload.SelectTab(1);
            ...
            if (isCheckingBoxingEnabledForSelection)
            {
                // 1. Find the last unchecked checkbox (start from the end of the list)
                int startIdx = allCheckBoxes.FindLastIndex(cb => !cb.Checked);
                if (startIdx == -1)
                    return; // All are checked

                // 2. Check up to the specified number of unchecked checkboxes in descending order
                int checkedCount = 0;
                for (int i = startIdx; i >= 0 && checkedCount < int.Parse(SelectTextBoxSelectionAmount.Text); i--)
                {
                    if (!allCheckBoxes[i].Checked)
                    {
                        allCheckBoxes[i].Checked = true;
            ...
            else
            {
                ...
                // 2. Check up to the specified number of unchecked checkboxes in descending order
                int unCheckedCount = 0;
                for (int i = startIdx; i >= 0 && unCheckedCount < int.Parse(SelectTextBoxSelectionAmount.Text); i--)
                {
                    if (allCheckBoxes[i].Checked)
                    {
                        allCheckBoxes[i].Checked = false;
        ...
        // Change button to Red if green and then green if red
        private void FilterSelectTextBoxSelectionAmountToggleButton_Click(object sender, EventArgs e)
        {
            if (FilterSelectTextBoxSelectionAmountToggleButton.BackColor == Color.LimeGreen)
            {
                FilterSelectTextBoxSelectionAmountToggleButton.BackColor = Color.Red;
                isCheckingBoxingEnabledForSelection = false;
            }
            else
            {
