/*
 * TC_1_97: VoWiFi MMS to VoWiFi Device Test Case
 * -----------------------------------------------
 * Purpose:
 *   Verify that an MMS sent from a VoWiFi device (camping on cellular) is received by another VoLTE device.
 * 
 * Steps:
 *   1. Confirm both devices are connected.
 *   2. Set Airplane mode ON, enable WiFi for both devices.
 *   3. Extract REF phone number.
...
namespace FIT_Automation.Test_Cases
{
    public class TC_1_97
    {
        ...
        public TC_1_97(string deviceId, RichTextBox outputRTB, Button testButton, string refDeviceId)
        {
            _deviceId = deviceId;
        ...
        public void RunTest()
        {
            ...
            lock (_lockObject)
            {
                ...
                if (!headerLogged)
                {
                    gclass.UpdateOutput("\n");
                    gclass.UpdateOutput("==================================================");
                    gclass.UpdateOutput("Starting TC 1.97: Verify MMS from VoWiFi device camping on cellular is sent to VoLTE device...");
                    gclass.UpdateOutput("==================================================\n");
                    headerLogged = true;
                }
            }
            ...
            try
            {
                ...
                if (!dutRegistered)
                {
                    ...
                    gclass.UpdateOutput($"TC 1.97: FAIL [{_deviceId}, {_refDeviceId}]", true);
                    gclass.LogTestResultToCSV("TC1.97", _deviceId, result);
                    return;
                }
                ...
                if (gclass.IsMMSSent)
                {
                    gclass.UpdateOutput($"TC 1.97: PASS [{_deviceId}, {_refDeviceId}]");
            ...
            {
                gclass.UpdateOutput($"TC 1.97: FAIL [{_deviceId}, {_refDeviceId}] - {ex.Message}", true);
                _testButton.BackColor = System.Drawing.Color.Red;
                result = "FAIL";
            }
            finally
