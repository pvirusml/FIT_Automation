/*
 * TC_1_94: Verify Call Waiting Notification for Incoming Video VoLTE Call
 * -----------------------------------------------------------------------
 * Purpose:
 *   Verify call waiting notification while on a Video VoLTE call for another incoming Video VoLTE call.
 * 
 * Steps:
 *   1. Ensure DUT 1 is IMS registered on LTE.
 *   2. Setup a video call between DUT 1 and DUT 2.
 *   3. From DUT 3, place a video call to DUT 1 while DUT 1 and DUT 2 are in an active video call.
...
namespace FIT_Automation.Test_Cases
{
    public class TC_1_94
    {
        ...
        public TC_1_94(string dut1Id, string dut2Id, string dut3Id, RichTextBox outputRTB, Button testButton)
        {
            _dut1Id = dut1Id;
            _dut2Id = dut2Id;
            _dut3Id = dut3Id;
        ...
        public void RunTest()
        {
            ...
            lock (_lockObject)
            {
                if (!headerLogged)
                {
                    gclass.UpdateOutput("\n");
                    gclass.UpdateOutput("==================================================");
                    gclass.UpdateOutput("Starting TC 1.94: Verify Call Waiting Notification for Incoming Video VoLTE Call");
                    gclass.UpdateOutput("==================================================\n");
                    headerLogged = true;
                }
            }
            ...
            try
            {
                ...
                gclass.RunAdbCommand($"adb -s {_dut3Id} shell am start -a android.intent.action.CALL -d tel:{dut1Number} --ei android.telecom.extra.START_CALL_WITH_VIDEO_STATE 3");
                ...
                gclass.UpdateOutput($"TC 1.94: PASS [{_dut1Id}, {_dut2Id}, {_dut3Id}]");
                _testButton.BackColor = Color.Green;
                result = "PASS";
            }
            catch (Exception ex)
            {
                gclass.UpdateOutput($"TC 1.94: FAIL [{_dut1Id}, {_dut2Id}, {_dut3Id}] - {ex.Message}", true);
            ...
            finally
            {
                ...
                gclass.LogTestResultToCSV("TC1.94", _dut1Id, result);
            }
        }
    }
}