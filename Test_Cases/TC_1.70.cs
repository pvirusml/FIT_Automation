/*
 * TC_1_70: MT SMS Reception over IMS Test Case
 * -------------------------------------------
 * Purpose:
 *   Verify MT SMS (on ICS) from Unison device is received using SIP over IMS
...
namespace FIT_Automation.Test_Cases
{
    public class TC_1_70
    {
        ...
        public TC_1_70(string deviceId, RichTextBox outputRTB, Button testButton, string refDeviceId)
        {
            _deviceId = deviceId;
        ...
        public void RunTest()
        {
            ...
            lock (_lockObject)
            {
                ...
                if (!headerLogged)
                {
                    gclass.UpdateOutput("\n");
                    gclass.UpdateOutput("==================================================");
                    gclass.UpdateOutput("Starting TC 1.70: Verify MT SMS (on ICS) from Unison device is received using SIP over IMS");
                    gclass.UpdateOutput("==================================================\n");
                    headerLogged = true;
                }
            }
            ...
            try
            {
                ...
                if (!gclass.WaitForLTEAndVoLTERegistration(_deviceId) || !gclass.WaitForLTEAndVoLTERegistration(_refDeviceId))
                {
                    ...
                    gclass.UpdateOutput($"TC 1.70: FAIL [{_deviceId}, {_refDeviceId}]", true);
                    gclass.LogTestResultToCSV("TC1.70", _deviceId, result);
                    return;
                }
                ...
                {
                    gclass.UpdateOutput($"TC 1.70: PASS [{_deviceId}, {_refDeviceId}]");
            ...
            {
                gclass.UpdateOutput($"TC 1.70: FAIL [{_deviceId}, {_refDeviceId}] - {ex.Message}", true);
                _testButton.BackColor = System.Drawing.Color.Red;
                result = "FAIL";
            }
